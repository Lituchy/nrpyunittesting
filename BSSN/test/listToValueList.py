from sympy import Integer, Symbol, symbols, simplify, Rational, Function, srepr, sin, cos, exp, log, Abs, Add, Mul, Pow, \
    preorder_traversal, N, Float, S, var, sympify, sqrt, sign, mathematica_code
...
def listToValueList(lst, first_time):
    ...
    for expr in lst:
        ...
        string2 = re.sub('Rational\(([0-9]+), ([0-9]+)\)',
                         "((Float('\\1'," + str(2 * precision) + "))/(Float('\\2'," + str(2 * precision) + ")))",
                         string)
    ...
    if first_time == True:
        ...
        for result in evaled_lst:
            if result != 0 and mpm.fabs(result) < 100 * 10 ** (-precision):
                print("Found |result| (" + str(
                    mpm.fabs(result)) + ") close to zero. Checking if indeed it should be zero.")
